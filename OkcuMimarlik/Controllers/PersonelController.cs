...
public class PersonelController : Controller
{
  ...
  [HttpPost]
    public IActionResult PersonelEkle(Personel model, IFormFile Personelİmage)
    {
     ...
     public IActionResult PersonelListesi(string searchString)
  {
     ...
     var personel = _context.Personels.ToList();
        ...
        if (!String.IsNullOrEmpty(searchString))
        {
          ...
          // personel = personel.Where(p => p.PersonelName != null && p.PersonelName.ToLower().Trim().Contains(searchString)).ToList();
        personel = personel.Where(p => p.PersonelName != null && p.PersonelName.IndexOf(searchString, StringComparison.OrdinalIgnoreCase) >= 0).ToList();

        }
    
           return View("PersonelListesi", personel); 
  }
